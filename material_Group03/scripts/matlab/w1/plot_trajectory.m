...
gt = readmatrix("ground_truth.csv");
xpos = gt(:,2);
ypos = gt(:,3);
xytimes = gt(:,1);
...
collisions = readmatrix("collisions.csv");
...
figure(1);
t_travel = max(xytimes)-min(xytimes); %%%%%%%%%% Improve
...
plot(xpos, ypos, "Color", "b", "Marker", "O");
